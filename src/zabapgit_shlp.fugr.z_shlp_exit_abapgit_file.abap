FUNCTION z_shlp_exit_abapgit_file.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  TABLES
...
* step.
  IF callcontrol-step = 'SELECT'.
    DATA lv_repokey TYPE ddshvalue.
    CALL FUNCTION 'F4UT_PARAMETER_VALUE_GET'
      ...
      EXCEPTIONS
        parameter_unknown = 1           " Parameter is not parameter of the search help
        OTHERS            = 2.

    IF lv_repokey IS INITIAL.
      callcontrol-step = 'EXIT'.
      EXIT.
    ENDIF.

    DATA(lo_online) = CAST zcl_abapgit_repo_online( zcl_abapgit_repo_srv=>get_instance( )->get( CONV #( lv_repokey ) ) ).
